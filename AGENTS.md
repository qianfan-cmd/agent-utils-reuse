...
6. **Confirm phase** (chat, before first Write tool — **not thinking-only**):
   ...
   - **Delta Confirm (v0.3.12)**: new import rows only; **newCall** (`Binding.method` on existing import) → method-level row — `placement-decision.md` §1.6.1
   - **Layered gate (v0.3.22)**: uiOnly / delta / newCall / sameSymbol
