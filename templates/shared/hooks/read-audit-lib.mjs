...
import { getAgentDir, hookErrorDenyMessageForRuntime } from './hook-runtime.mjs'
...
function agentDirPath(cwd = process.cwd()) {
  return path.join(cwd, getAgentDir())
}

export function auditPath(cwd = process.cwd()) {
  return path.join(agentDirPath(cwd), AUDIT_FILENAME)
}
...
export function logHookError(cwd, context, err) {
  try {
    const filePath = path.join(agentDirPath(cwd), HOOK_ERROR_LOG)
...
export function hookErrorDenyMessage(detail) {
  return hookErrorDenyMessageForRuntime(detail, getAgentDir())
}
...
export function logHookGateDebug(cwd, context, meta = {}) {
  try {
    const filePath = path.join(agentDirPath(cwd), HOOK_DEBUG_LOG)
...
export function verdictAuditPath(cwd = process.cwd()) {
  return path.join(agentDirPath(cwd), VERDICT_AUDIT_FILENAME)
}
...
export function agentsReadAuditPath(cwd = process.cwd()) {
  return path.join(agentDirPath(cwd), AGENTS_READ_AUDIT_FILENAME)
}
...
export function discoveryAuditPath(cwd = process.cwd()) {
  return path.join(agentDirPath(cwd), DISCOVERY_AUDIT_FILENAME)
}
