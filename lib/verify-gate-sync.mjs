...
function expectedHooksJsonContent(templatesRoot) {
  const fragmentPath = path.join(templatesRoot, 'hooks.json.fragment')
  const fragment = JSON.parse(fs.readFileSync(fragmentPath, 'utf8'))
  const merged = mergeHooksJson({}, fragment, { force: true })
...
export function verifyGateSync(templatesRoot, projectRoot, config) {
  ...
  const expectedHooks = expectedHooksJsonContent(templatesRoot)
