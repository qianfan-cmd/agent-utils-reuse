...
import { readHooksFragment } from './build-hooks-json.mjs'
import { serializeHooksJson } from './build-hooks-json.mjs'
import { getAgentTarget, sessionAuditRelPaths, PACKAGE_HOOK_WRAPPER_FILES, SHARED_HOOK_CORE_FILES } from './agent-targets.mjs'
...
import {
  mergeInstalledAgentTargets,
  readInstalledAgentTargets
} from './resolve-agent-targets.mjs'
import { stripClaudeGateHooks, serializeClaudeSettings } from './merge-claude-settings.mjs'
...
const SESSION_AUDIT_REL_PATHS = [
  ...sessionAuditRelPaths('cursor'),
  ...sessionAuditRelPaths('claude'),
  ...sessionAuditRelPaths('codex')
]
...
function listGateTargetInstallRelPaths(templatesRoot, targetId) {
  const target = getAgentTarget(targetId)
  const src = path.join(templatesRoot, target.templateSubdir)
  if (!fs.existsSync(src)) return []
  const rels = walkRelFiles(src, src)
  return rels.map((rel) => `${target.agentDir}/${rel}`)
}

/** Mirror init copyDir(templates/cursor → .cursor) for uninstall. */
export function listGateCursorInstallRelPaths(templatesRoot) {
  return listGateTargetInstallRelPaths(templatesRoot, 'cursor')
}

export function buildUninstallFileList(projectRoot, config, templatesRoot, targets = ['cursor']) {
  const relPaths = new Set()

  for (const targetId of targets) {
    const target = getAgentTarget(targetId)
    for (const rel of listGateTargetInstallRelPaths(templatesRoot, targetId)) {
      relPaths.add(rel)
    }
    for (const name of PACKAGE_HOOK_WRAPPER_FILES) {
      relPaths.add(`${target.agentDir}/hooks/${name}`)
    }
    for (const name of SHARED_HOOK_CORE_FILES) {
      relPaths.add(`${target.agentDir}/hooks/_shared/${name}`)
    }
    if (targetId === 'cursor') {
      relPaths.add('.cursor/hooks/read-audit-lib.mjs')
    }
    if (targetId === 'claude') {
      for (const name of PACKAGE_RULE_FILES) {
        relPaths.add(`.claude/rules/${name.replace(/\.mdc$/, '.md')}`)
      }
    }
    relPaths.add(`${target.skillsDir}/reuse-before-create/SKILL.md`.replace(/\\/g, '/'))
    for (const rel of sessionAuditRelPaths(targetId)) {
      relPaths.add(rel)
    }
  }

  const installed = readInstalledAgentTargets(config)
  const remaining = installed.filter((id) => !targets.includes(id))
  const removeShared = remaining.length === 0
  ...
  return { paths: [...relPaths].sort(), removeShared, remaining }
}
...
export async function runUninstall(cwd, options = {}) {
  const { yes = false, dryRun = false, targets: targetsOpt = ['cursor'] } = options
  ...
  try {
    ...
    config = {
      ...
      installedAgentTargets: ['cursor']
    }
    gateInstalled = false
  }

  const templatesRoot = resolveTemplatesRoot(projectRoot)
  const targets = targetsOpt
  const { paths: fileList, removeShared, remaining } = buildUninstallFileList(
    projectRoot,
    config,
    templatesRoot,
    targets
  ...
  const summaryLines = [
    ...
    `  Files/dirs to remove: ${fileList.length} path(s)${removeShared ? ' + utils-book dir' : ''}`,
    removeShared
      ? '  package.json: remove agent-utils-reuse dependency + gate scripts'
      : '  package.json: unchanged (other IDE targets remain)',
    removeShared
      ? '  AGENTS.md / project rule: strip marker blocks'
      : '  AGENTS.md: kept (shared targets remain)'
  ]
  ...
  const bookRemoved = removeShared ? removeDirIfExists(projectRoot, bookDirRel, dryRun) : null
  ...
  if (removeShared) {
    agentsResult = unmergeAgentsMd(projectRoot, config.agentsFile, { dryRun })
  ...
  let ruleResult = { action: 'skipped' }
  if (removeShared) {
    ruleResult = unmergeProjectAgentCoreRule(projectRoot, config.projectAgentCoreRule, { dryRun })
  ...
  for (const targetId of targets) {
    if (targetId === 'cursor') {
      ...
      if (fs.existsSync(hooksPath) && templatesRoot) {
        try {
          const fragment = readHooksFragment(templatesRoot, 'cursor')
    ...
    if (targetId === 'claude') {
      ...
      if (fs.existsSync(settingsPath) && templatesRoot) {
        try {
          const fragment = readHooksFragment(templatesRoot, 'claude')
          const existing = JSON.parse(fs.readFileSync(settingsPath, 'utf8'))
          const stripped = stripClaudeGateHooks(existing, fragment)
          if (!dryRun) {
            fs.writeFileSync(settingsPath, serializeClaudeSettings(stripped), 'utf8')
          }
    ...
    if (targetId === 'codex') {
      ...
      if (fs.existsSync(hooksPath) && templatesRoot) {
        try {
          const fragment = readHooksFragment(templatesRoot, 'codex')
          const existing = JSON.parse(fs.readFileSync(hooksPath, 'utf8'))
          const stripped = mergeCodexHooksJson(existing, fragment, 'off')
          if (!dryRun) {
            fs.writeFileSync(hooksPath, serializeCodexHooksJson(stripped), 'utf8')
          }
          modified.push('.codex/hooks.json')
        } catch (err) {
          warnings.push(`codex hooks.json: ${err.message}`)
        }
      }
    }
  }
  ...
  for (const targetId of targets) {
    const target = getAgentTarget(targetId)
    const dirRel = target.agentDir.replace(/\\/g, '/')
    const dirRemoved = removeDirIfExists(projectRoot, dirRel, dryRun)
    if (dirRemoved) removed.push(dirRel)
    if (targetId === 'codex') {
      const skillRel = `${target.skillsDir}/reuse-before-create`.replace(/\\/g, '/')
      const skillRemoved = removeDirIfExists(projectRoot, skillRel, dryRun)
      if (skillRemoved) removed.push(skillRel)
    }
  }

  if (!dryRun && gateInstalled) {
    const nextTargets = remaining
    if (removeShared) {
      removeFileIfExists(projectRoot, CONFIG_FILENAME, dryRun)
      removed.push(CONFIG_FILENAME)
    } else {
      const bookrcPath = path.join(projectRoot, CONFIG_FILENAME)
      if (fs.existsSync(bookrcPath)) {
        const raw = JSON.parse(fs.readFileSync(bookrcPath, 'utf8'))
        raw.installedAgentTargets = nextTargets
        fs.writeFileSync(bookrcPath, `${JSON.stringify(raw, null, 2)}\n`, 'utf8')
        modified.push(`${CONFIG_FILENAME} (installedAgentTargets)`)
      }
    }
  }
  ...
  let nextPkg = pkg
  if (removeShared) {
    nextPkg = removeGatePackageScripts(pkg)
