...
function listCatalogGateRelPaths(config) {
  const rels = new Set()
  for (const name of MERGEABLE_GATE_DOC_FILES) {
    rels.add(`${config.catalogDir}/${name}`.replace(/\\/g, '/'))
  }
  rels.add(`${config.catalogDir}/AGENTS.utils-reuse.snippet.md`.replace(/\\/g, '/'))
  rels.add(config.utilsIndexFile.replace(/\\/g, '/'))
  return rels
}
...
export async function runUninstall(cwd, options = {}) {
  ...
  const catalogGatePaths = listCatalogGateRelPaths(config)
  ...
  if (fs.existsSync(catalogAbs)) {
    ...
    const unknown = remaining.filter((name) => {
      const rel = `${config.catalogDir}/${name}`.replace(/\\/g, '/')
      if (catalogGatePaths.has(rel)) return false
