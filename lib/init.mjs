...
import { mergeBookrc, syncPackageFiles } from './sync-package-files.mjs'
...
function writeConfig(projectRoot, raw, force) {
  return mergeBookrc(projectRoot, raw, { force })
}
...
function patchHooksJson(projectRoot, force) {
  ...
  const merged = mergeHooksJson(existing, fragment, { force })
...
function patchGitignore(projectRoot) {
  ...
  const lines = ['.cursor/.utils-gate-reads.json', '.cursor/.utils-gate-verdict.json']
...
export function runInit(cwd, options = {}) {
  const { yes = false, force = false, withExamples = false, forceDocs = false } = options
  ...
  const packageSync = syncPackageFiles(TEMPLATES_ROOT, projectRoot, config, { forceDocs })
  ...
  const hooksPath = patchHooksJson(projectRoot, true)
  ...
  const projectGateSnippetPath = path.join(TEMPLATES_ROOT, 'snippets', 'project-agent-gate.inject.md')
...
export function printInitSummary(result) {
  ...
  if (docsSync?.skippedCustomized?.length) {
    console.log(`  Docs:   refreshed ${docsSync.refreshed?.length ?? 0}, skipped customized ${docsSync.skippedCustomized.length}`)
    for (const rel of docsSync.skippedCustomized) {
      console.log(`          - ${rel}`)
    }
  } else {
  ...
  if (gitignoreResult?.written) {
    console.log(`  Gitignore: added .cursor/.utils-gate-reads.json`)
  }
  console.log('')
  ...
  console.log('')
  console.log('  Hook: confirm (default) — deny Write until util Read + prior-chat Verdict')
  console.log('  Optional: set hookMode to remind in .utils-bookrc.json (see README)')
  console.log('')
