...
function patchGitignore(projectRoot) {
  ...
  const line = '.cursor/.utils-gate-reads.json'
  ...
  if (content.includes(line)) {
    return { written: false, path: gitignorePath }
  }
...
export function runInit(cwd, options = {}) {
  ...
  const packageSync = syncPackageFiles(TEMPLATES_ROOT, projectRoot, config)
...
export function printInitSummary(result) {
  ...
  console.log('')
  console.log('  Hook: confirm (default) — deny Write until util file Read; chat Verdict still required')
