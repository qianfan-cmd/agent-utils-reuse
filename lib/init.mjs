...
import { patchHooksForTarget } from './build-hooks-json.mjs'
import { mergePackageScripts } from './merge-json.mjs'
import { GITIGNORE_AUDIT_LINES } from './gitignore-audit.mjs'
import {
  copyTargetTemplateTree,
  ensureTargetTemplateArtifacts,
  installTargetExtras
} from './install-agent-target.mjs'
import { mergeInstalledAgentTargets } from './resolve-agent-targets.mjs'
...
export function patchHooksJson(projectRoot, templatesRoot, { force = false, hookMode = 'off' } = {}) {
  return patchHooksForTarget(projectRoot, templatesRoot, 'cursor', hookMode)
}

export function patchHooksForTargets(projectRoot, templatesRoot, targets, hookMode = 'off') {
  const paths = {}
  for (const targetId of targets) {
    paths[targetId] = patchHooksForTarget(projectRoot, templatesRoot, targetId, hookMode)
  }
  return paths
}
...
export function runInit(cwd, options = {}) {
  ...
  ensureTargetTemplateArtifacts(templatesRoot)

  const raw = defaultConfigRaw()
  const configResult = mergeBookrc(projectRoot, raw, { force })
  const existingRaw = JSON.parse(fs.readFileSync(configResult.path, 'utf8'))
  const installedAgentTargets = mergeInstalledAgentTargets(
    existingRaw.installedAgentTargets,
    targets
  )
  mergeBookrc(projectRoot, { installedAgentTargets }, { force: false })
  ...
  for (const targetId of targets) {
    targetCopies[targetId] = copyTargetTemplateTree(templatesRoot, projectRoot, targetId, {
      force
    })
    hooksPaths[targetId] = patchHooksForTarget(
      projectRoot,
      templatesRoot,
      targetId,
      config.hookMode
    )
    targetExtras[targetId] = installTargetExtras(projectRoot, targetId, { force })
  }
  ...
  return {
    ...
    targets,
    targetCopies,
    cursorCopy: targetCopies.cursor ?? { copied: 0, skipped: 0 },
    examplesCopy,
    scripts,
    hooksPath: hooksPaths.cursor ?? hooksPaths[targets[0]],
    hooksPaths,
    targetExtras,
    gitignoreResult,
    agentsResult,
...
export function printInitSummary(result) {
  ...
  console.log(`  Agent targets: ${targets.join(', ')}`)
  for (const targetId of targets) {
    const copy = targetCopies[targetId] ?? { copied: 0, skipped: 0 }
    console.log(`  ${targetId}: copied ${copy.copied}, skipped ${copy.skipped} (existing)`)
    if (hooksPaths[targetId]) {
      console.log(`    hooks: ${hooksPaths[targetId]}`)
    }
    const extra = targetExtras[targetId]
    if (extra?.action && extra.action !== 'skipped') {
      console.log(`    extra: ${extra.action} ${extra.path ?? ''}`)
    }
  }
  ...
  console.log('')
  console.log('  Open your agent IDE at this project root (Cursor / Claude Code / Codex).')
  console.log('')
  ...
  console.log('  2. Missing @utils-book on existing exports?')
  console.log('       README — Backfill @utils-book section (copy-paste Agent prompt)')
  console.log('       Or: node_modules/agent-utils-reuse/docs/en/backfill-jsdoc.md')
  console.log('       Or: node_modules/agent-utils-reuse/docs/zh-CN/backfill-jsdoc.md')
  console.log('     Agent prompt files: docs/agent-catalog/BACKFILL-UTILS-BOOK.en.md | .zh.md')
  console.log('  3. Agent reuse rules: docs/agent-catalog/placement-decision.md')
  console.log('  4. Upgrade package + gate later: pnpm upgrade:utils-reuse')
  console.log('  5. Optional CI: pnpm check:utils-book (commit utils-book + utils-index first)')
  console.log('')
}
