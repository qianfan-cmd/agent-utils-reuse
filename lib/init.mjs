...
export function printInitSummary(result) {
  ...
  if (withExamples && !examplesCopy.missing && examplesCopy.copied > 0) {
    console.log('  2. Example utils copied to src/utils/array — run: pnpm gen:utils-book')
  } else {
    console.log('  2. Add utils under utilsDir (or re-run init --with-examples), then: pnpm gen:utils-book')
  }
  console.log('  3. Optional JSDoc @utils-book tags improve Shortlist summaries')
