...
export const WORKFLOW_SECTION_SKELETON = `### 7. Agent workflow
...
export function stripWorkflowInjectBlocks(content) {
  let next = content
  next = next.replace(
    /<!-- agent-utils-reuse:workflow-inject -->[\s\S]*?<!-- agent-utils-reuse:workflow-inject -->\s*/g,
    ''
  )
  next = next.replace(
    /(\s*<!-- agent-utils-reuse:workflow-inject -->\s*\n\s*- If shared utils[^\n]*\n)+/g,
    '\n'
  )
  return next
}
...
export function injectAgentsWorkflow(projectRoot, agentsFile, { force = false } = {}) {
  ...
  content = stripWorkflowInjectBlocks(content)
  ...
  if (!injected) {
    ...
    content = `${content}${separator}${WORKFLOW_SECTION_SKELETON}\n`
    injected = true
  }
