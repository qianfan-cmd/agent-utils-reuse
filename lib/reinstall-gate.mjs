...
import {
  patchGitignore,
  patchHooksForTargets,
  patchPackageJson,
  readNodeModulesPackageVersion
} from './init.mjs'
import { ensureTargetTemplateArtifacts } from './install-agent-target.mjs'
import { readInstalledAgentTargets } from './resolve-agent-targets.mjs'
import {
  compareSemver,
  mergeBookrc,
  pruneBookrcObsolete,
...
export function runGateReinstall(cwd, options = {}) {
  const { dryRun = false, acceptUpstream = false, targets: targetsOpt = null } = options
  ...
  const targets = targetsOpt ?? readInstalledAgentTargets(config)
  ...
  if (!dryRun) {
    hooksPaths = patchHooksForTargets(projectRoot, templatesRoot, targets, config.hookMode)
    ...
    for (const targetId of targets) {
      const sub = pruneOrphanHookFiles(projectRoot, targetId, { dryRun: false })
      orphanHooksResult.removed.push(...sub.removed)
    }
  }
  ...
  if (!dryRun) {
    verifyResult = verifyGateSync(templatesRoot, projectRoot, config, targets)
...
export function printGateReinstallSummary(result) {
  ...
  if (!dryRun) {
    console.log(`  Agent targets: ${(result.targets ?? ['cursor']).join(', ')}`)
    for (const [targetId, p] of Object.entries(hooksPaths)) {
      console.log(`  Hooks (${targetId}): ${p}`)
    }
