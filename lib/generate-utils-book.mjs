...
function createGenerator(config) {
  ...
  function renderIndex(chapters, fileRecords, ambiguous, stats) {
    ...
    const lines = [
      ...
      '> **DO NOT EDIT** — 由 `pnpm gen:utils-book` 生成。',
      '',
      '> **HUMAN ONLY** — Cursor Agent **禁止** Read 本文件或 `' + bookRel + '/` 下章 md 做 Shortlist。',
      '> Agent Discovery D1：`' + indexRel + '` + `agent-utils-reuse search "<keywords>"`（见 placement-decision.md §2）。',
      '',
      '## 人类浏览（Code Review / 文档）',
      '',
  ...
  function buildSearchText(name, chapter, fileRel, summary, config) {
    const parts = [name]
    if (name.includes('.')) parts.push(name.split('.').pop())
    parts.push(chapter, fileRel, summary)
    const synonyms = config.searchSynonyms ?? {}
    for (const [kw, aliases] of Object.entries(synonyms)) {
      const aliasList = Array.isArray(aliases) ? aliases : [aliases]
      const nameLower = name.toLowerCase()
      const summaryLower = String(summary ?? '').toLowerCase()
      if (
        nameLower.includes(String(kw).toLowerCase()) ||
        summaryLower.includes(String(kw).toLowerCase())
      ) {
        parts.push(...aliasList.map(String))
      }
    }
    return parts.join(' ').replace(/\s+/g, ' ').trim()
  }

  function renderUtilsIndex(fileRecords, ambiguous, stats) {
    const symbols = {}
    for (const file of fileRecords) {
      const chapter = chapterFromRel(file.rel)
      for (const e of file.entries) {
        if (e.name === '*' || e.kind === 're-export-all') continue
        const summary = e.summary || NO_SUMMARY
        const entry = {
          path: file.path,
          line: e.line,
          summary,
          chapter,
          kind: e.kind,
          searchText: buildSearchText(e.name, chapter, file.rel, summary, config)
        }
        if (!symbols[e.name]) symbols[e.name] = []
        symbols[e.name].push(entry)
      }
    }

    const ambiguousObj = {}
    for (const [sym, paths] of ambiguous) {
      const unique = [...new Map(paths.map((p) => [p.path, p])).values()]
      ambiguousObj[sym] = unique.map((p) => ({
        symbol: p.symbol,
        path: p.path
      }))
    }

    const siblingsByPath = {}
    ...
    for (const [p, names] of byPath) {
      ...
      if (unique.length >= 2) siblingsByPath[p] = unique
    }

    const crossFileSiblings = {}
    const groups = config.crossFileSiblingGroups ?? []
    for (const group of groups) {
      if (!Array.isArray(group) || group.length < 2) continue
      for (const sym of group) {
        crossFileSiblings[sym] = group.filter((s) => s !== sym)
      }
    }
...
export function generateUtilsBook(config, { check = false } = {}) {
  ...
  const utilsIndex = gen.renderUtilsIndex(fileRecords, ambiguous, stats)
  const indexOutPath = config.indexFilePath
  fs.mkdirSync(path.dirname(indexOutPath), { recursive: true })
  fs.writeFileSync(indexOutPath, `${JSON.stringify(utilsIndex, null, 2)}\n`, 'utf8')

  const indexRel = path.relative(config.projectRoot, indexOutPath).replace(/\\/g, '/')
  console.log(`Wrote utils-book/index.md + ${chapters.size} chapters`)
  console.log(`Wrote ${indexRel} (${stats.totalSymbols} symbols)`)
