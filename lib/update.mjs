...
import { scanIndexHealth, formatIndexHealthSummary } from './index-health.mjs'
...
export function runUpgrade(cwd, options = {}) {
  ...
  const resolvedTargets = targets ?? readInstalledAgentTargets(config)
...
export function runUpdate(cwd, options = {}) {
  ...
  if (dryRun) {
    ...
    try {
      ...
      verifyResult = verifyGateSync(templatesInfo.templatesRoot, projectRoot, config, resolvedTargets)
    } catch {
      templatesInfo = null
    }
...
export function runVerify(cwd, options = {}) {
  const { targets = null } = options
  const projectRoot = path.resolve(cwd)
  const config = loadConfig(projectRoot)
  const resolvedTargets = targets ?? readInstalledAgentTargets(config)
  const templatesInfo = resolveTemplatesRootInfo(projectRoot)
  const verifyResult = verifyGateSync(
    templatesInfo.templatesRoot,
    projectRoot,
    config,
    resolvedTargets
  )
...
export function runStatus(cwd) {
  ...
  const indexHealth = scanIndexHealth(projectRoot, config)
...
export function printStatusSummary(result) {
  ...
  if (result.indexHealth) {
    console.log('')
    for (const line of formatIndexHealthSummary(result.indexHealth)) {
      console.log(`  ${line}`)
    }
  }
