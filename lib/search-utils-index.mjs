...
function siblingsForPath(index, filePath, symbolName) {
  ...
  const fromPath = Array.isArray(siblings) && siblings.length >= 2
    ? siblings.filter((s) => s !== symbolName)
    : []
  const cross = index.crossFileSiblings?.[symbolName] ?? []
  return [...new Set([...fromPath, ...cross])]
}
  ...
  if (json) {
    return JSON.stringify(
      ...
      if (index) {
        ...
        const cross = index.crossFileSiblings?.[name]
        if (cross?.length > 0) row.crossFileSiblings = cross
      }
        return row
      }),
      null,
    ...
    .map((r) => {
      ...
      if (index) {
        ...
        if (siblings.length > 0) {
          const sameFile = (index.siblingsByPath?.[r.path] ?? []).filter((s) => s !== r.name)
          if (sameFile.length > 0) {
            line += `\n  siblings @ ${r.path}: ${sameFile.join(', ')}`
          }
          const cross = index.crossFileSiblings?.[r.name] ?? []
          if (cross.length > 0) {
            line += `\n  cross-file siblings: ${cross.join(', ')}`
          }
        }
      }
      return line
