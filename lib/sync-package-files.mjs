...
import { syncPackageDocs } from './sync-docs.mjs'
...
export function syncPackageFiles(templatesRoot, projectRoot, config, options = {}) {
  const { forceDocs = false, dryRun = false } = options
  let copied = 0
  const details = []
  ...
  const docsResult = syncPackageDocs(templatesRoot, projectRoot, config, { forceDocs, dryRun })
  copied += docsResult.copied
  for (const rel of [...docsResult.created, ...docsResult.refreshed]) {
    details.push(rel)
  }
